...
function parseCoords(latValue,lngValue){const lat=Number(latValue),lng=Number(lngValue);if(!Number.isFinite(lat)||!Number.isFinite(lng)||lat<-90||lat>90||lng<-180||lng>180)throw httpError(400,'Некорректные координаты');return {lat,lng}}
function pathSafe(urlPath){const decoded=decodeURIComponent(urlPath.split('?')[0]);const rel=decoded==='/'?'index.html':decoded.replace(/^\/+/, '');const full=path.normalize(path.join(PUBLIC,rel));return full.startsWith(PUBLIC)?full:null}
function serveStatic(req,res){const full=pathSafe(req.url);if(!full)return json(res,403,{error:'Forbidden'});fs.stat(full,(err,stat)=>{if(err||!stat.isFile())return json(res,404,{error:'Not found'});const ext=path.extname(full).toLowerCase();const headers={'content-type':mime[ext]||'application/octet-stream','x-content-type-options':'nosniff','referrer-policy':'same-origin'};if(/\.(png|gif|jpg|jpeg|svg|mp3)$/.test(ext))headers['cache-control']='public, max-age=86400';else headers['cache-control']='no-cache';res.writeHead(200,headers);fs.createReadStream(full).pipe(res)})}
...
async function api(req,res,url){
  ...
  const result=await store.transaction(async state=>{
    ...
    if(method==='GET'&&url.pathname==='/api/world'){const {lat,lng}=parseCoords(url.searchParams.get('lat'),url.searchParams.get('lng'));return {objects:worldNearby(state,p,lat,lng,Number(url.searchParams.get('radius')||1600)),district:publicDistrict(getDistrict(state,lat,lng))}}
    ...
    if(method==='POST'&&url.pathname==='/api/encounters/resolve'){
      const body=await readJson(req),challenge=state.encounters[body.challengeId],out=resolveEncounter(state,p,body);
      if(out.success&&challenge?.obj){const districtResult=contributeToDistrict(state,p,{lat:challenge.obj.lat,lng:challenge.obj.lng,choice:body.choice,eventType:challenge.obj.type,amount:out.reward?.contribution||1});out.district=districtResult.district;out.districtContribution=districtResult.contribution;broadcast('district:update',{districtId:districtResult.district.id,district:districtResult.district,resolvedNow:districtResult.resolvedNow})}
      if(out.reward?.shared)broadcast('rift:update',{eventId:out.reward.eventId,...out.reward.shared});return out;
    }
